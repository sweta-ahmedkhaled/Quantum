...

{
        ...
        int FIG2; // whether to use figure-2 timing mode (measurement time only)
...

{
    ...
    time_t tp;          // used by original DISPTIME mode (seconds resolution)
    double dt;
    char mvirgin = 1;


    // Edited by Ahmed Khaled @ 1-1-2026
    // Figure-2 timing (higher resolution than time())
    clock_t meas_start = 0;
    ...
    for (t = 0; t < h->T; t++)
    {
        ...
        if (h->a[t] == MEASURE)
        {
            // Start measurement-only timer at the FIRST measurement
            if (!meas_started)
            {
                meas_started = 1;
                meas_start = clock();
            ...
            // In normal mode, print gate timing once at the first measurement (original behavior)
            if (!h->FIG2)
            {
            ...
            // In normal mode, print measurement outcomes unless SILENT
            if (!h->FIG2 && !h->SILENT)
            {
        ...
        // In normal mode, optionally print each executed instruction
        if (!h->FIG2 && h->DISPPROG)
        {
    ...
    // -----------------------------
    // FIGURE-2 MODE: print ONLY a single number (measurement time in seconds)
    // -----------------------------
    if (h->FIG2)
    {
        if (meas_started)
            meas_seconds = ((double)(clock() - meas_start)) / CLOCKS_PER_SEC;
        else
            meas_seconds = 0.0;

        // IMPORTANT: one clean numeric line for Python parsing
        printf("%.10f\n", meas_seconds);
        return;
    }
...

{
        ...
        if (params)
	{
         ...
         for (t = 1; t < l; t++)
         {
                 ...
                 if ((params[t]=='f')||(params[t]=='F')) h->FIG2 = 1;
         }
	}
